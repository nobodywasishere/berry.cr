...
typedef void (*writebuffer_callback_t)(const char *buffer, size_t length, void *io);
typedef char *(*readstring_callback_t)(const char *buffer, size_t length, void *io);

typedef void *writebuffer_io_t;
typedef void *readstring_io_t;

writebuffer_callback_t writebuffer_callback;
readstring_callback_t readstring_callback;
writebuffer_io_t writebuffer_io;
readstring_io_t readstring_io;

BERRY_API void be_writebuffer_callback_register(writebuffer_callback_t callback, writebuffer_io_t io)
{
    writebuffer_callback = callback;
    writebuffer_io = io;
}

BERRY_API void be_writebuffer(const char *buffer, size_t length)
{
    if (writebuffer_callback)
    {
        writebuffer_callback(buffer, length, writebuffer_io);
    }
    else
    {
...
BERRY_API void be_readstring_callback_register(readstring_callback_t callback, readstring_io_t io)
{
    readstring_callback = callback;
    readstring_io = io;
}

BERRY_API char *be_readstring(char *buffer, size_t size)
{
    if (readstring_callback)
    {
        return readstring_callback(buffer, size, readstring_io);
    }
    else
    {
...
int be_isdir(const char *path)
{
    ...
    return type != INVALID_FILE_ATTRIBUTES && (type & FILE_ATTRIBUTE_DIRECTORY) != 0;
}
